...
class MySqlDatabase : public DatabaseInterface
{
 ...
 private:
  ...
  std::map<std::string, std::vector<std::shared_ptr<const o2::quality_control::core::QualityObject>>> mQualityObjectsQueue;
  std::map<std::string, std::vector<std::shared_ptr<const o2::quality_control::core::MonitorObject>>> mMonitorObjectsQueue;
  size_t queueSize;
