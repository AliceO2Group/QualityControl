...
BOOST_AUTO_TEST_CASE(mo_save)
{
  ...
  ILOG << LogInfoSupport << "getName : '" << obj.getName() << "'" << ENDM;
  ILOG << LogInfoSupport << "GetName : '" << obj.GetName() << "'" << ENDM;
  ILOG << LogInfoSupport << "title : '" << obj.GetTitle() << "'" << ENDM;
  ...
  ILOG << LogInfoSupport << "mo : " << mo << ENDM;
  BOOST_CHECK_EQUAL(mo->GetName(), objectName);
  BOOST_CHECK_EQUAL(mo->getName(), objectName);
  ILOG << LogInfoSupport << "name : " << mo->GetName() << ENDM;
  ILOG << LogInfoSupport << "name : " << mo->getName() << ENDM;
