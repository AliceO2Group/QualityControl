...
{
  auto dataProcessorName = DataProcessorAdapter::dataProcessorName<LateTaskRunner>(taskConfig.name, taskConfig.detectorName);
  auto inputs = DataProcessorAdapter::collectUserInputs<LateTaskRunner>(taskConfig);
  auto outputs = DataProcessorAdapter::collectUserOutputs<LateTaskRunner, DataSourceType::LateTask>(taskConfig);
...
LateTaskConfig LateTaskRunnerFactory::extractConfig(const CommonSpec& commonSpec, const LateTaskSpec& lateTaskSpec)
{
  ...
  return LateTaskConfig{
    ...
    lateTaskSpec.outputActivityStrategy
  };
}

void LateTaskRunnerFactory::customizeInfrastructure(std::vector<framework::CompletionPolicy>& policies)
{
  auto matcher = [label = DataProcessorAdapter::dataProcessorLabel<LateTaskRunner>()](auto const& device) {
