...
std::optional<double> string2Seconds(std::string str)
{
  ...
  try {
    ...
    ILOG(Error, Support) << "Unexpected format of string describing time '" << str << "'" << ENDM;
    throw ex;
  } catch (std::out_of_range& ex) {
    ILOG(Error, Support) << "Trying to convert time, which is out of supported range '" << str << "'" << ENDM;
    throw ex;
  }
}
...
TriggerFcn triggerFactory(const std::string& trigger, const PostProcessingConfig& config)
{
  ...
  if (triggerLowerCase == "once") {
    ...
    const std::string& dbUrl = db == "qcdb" ? config.database.at("host") : config.conditionUrl;
