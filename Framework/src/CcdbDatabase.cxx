...
void CcdbDatabase::storeMO(std::shared_ptr<o2::quality_control::core::MonitorObject> mo)
{
  ...
  ILOG << LogDebugSupport << "Storing MonitorObject " << path << ENDM;
...
void CcdbDatabase::storeQO(std::shared_ptr<QualityObject> qo)
{
  ...
  ILOG << LogDebugSupport << "Storing object " << path << ENDM;
...
TObject* CcdbDatabase::retrieveTObject(std::string path, std::map<std::string, std::string> const& metadata, long timestamp, std::map<std::string, std::string>* headers)
{
  ...
  if (object == nullptr) {
    ...
    if (object == nullptr) {
      ILOG << LogErrorSupport << "We could NOT retrieve the object " << path << "." << ENDM;
      return nullptr;
    }
  }
  ILOG << LogDebugSupport << "Retrieved object " << path << " with timestamp " << timestamp << ENDM;
  return object;
}
...
std::string CcdbDatabase::retrieveJson(std::string path, long timestamp, const std::map<std::string, std::string>& metadata)
{
  ...
  if (toConvert == nullptr) {
    ILOG << LogErrorSupport << "Unable to get the object to convert" << ENDM;
    return std::string();
  }
  ...
  if (jsonDocument.Parse(json.Data()).HasParseError()) {
    ILOG << LogErrorSupport << "Unable to parse the JSON returned by TBufferJSON for object " << path << ENDM;
    return std::string();
  }
