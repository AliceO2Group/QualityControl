...
class ObjectVersion:
    ...
    def __repr__(self):
        if "Run" in self.metadata or "RunNumber" in self.metadata:
            ...
            return f"Version of object {self.path} valid from {self.valid_from_as_dt}, run {run_number} (uuid {self.uuid})"
        else:
            return f"Version of object {self.path} valid from {self.valid_from_as_dt} (uuid {self.uuid}, " \
