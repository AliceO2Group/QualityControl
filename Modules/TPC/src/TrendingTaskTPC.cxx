...
void TrendingTaskTPC::initialize(Trigger, framework::ServiceRegistry&)
{
  ...
  // Define the branch for each datasource (TH1, TH2, canvas,...) in the config.
...
void TrendingTaskTPC::trendValues(uint64_t timestamp, repository::DatabaseInterface& qcdb)
{
  ...
  for (auto& dataSource : mConfig.dataSources) {
    ...
    // Check if the dataSource is a canvas (name starts with "c_") or not.
    // Return true if it is a canvas.
    bool isDataCanvas = boost::algorithm::starts_with(dataSource.name, "c_");
    ...
    if (dataSource.type == "repository") {
      ...
      if (obj) {
        mReductors[dataSource.name]->update(obj, dataSource.axisDivision, isDataCanvas);
        ...
        if (isDataCanvas) {
          ...
          mNumberPads = dynamic_cast<TList*>(canvas->GetListOfPrimitives())->GetEntries();
        }
      }
    } else if (dataSource.type == "repository-quality") {
      auto qo = qcdb.retrieveQO(dataSource.path + "/" + dataSource.name, timestamp);
      if (qo) {
        mReductors[dataSource.name]->update(qo.get(), dataSource.axisDivision, isDataCanvas);
      }
    } else {
