...
void DigitQcTask::initialize(o2::framework::InitContext& /*ctx*/)
{
  ...
  */
  ...
  mHistAmp2Ch = std::make_unique<TH2F>("AmpPerChannel", "Amplitude vs Channel;Amp;Channel;", sNCHANNELS_PM, 0, sNCHANNELS_PM, 4200, -100, 4100);
...
void DigitQcTask::monitorData(o2::framework::ProcessingContext& ctx)
{
  ...
  for (auto& digit : digits) {
    ...
    */
    ...
    fit::Triggers::DEFAULT_AMP && digit.mTriggers.getTimeA() == fit::Triggers::DEFAULT_ZERO && digit.mTriggers.getTimeC() == fit::Triggers::DEFAULT_ZERO)
      isTCM = false;
    ...
    for (const auto& chData : vecChData) {
      mHistTime2Ch->Fill(static_cast<Double_t>(ch_data::getTime(chData)), static_cast<Double_t>(ch_data::getChId(chData)));
      mHistAmp2Ch->Fill(static_cast<Double_t>(ch_data::getChId(chData)),static_cast<Double_t>(ch_data::getCharge(chData)));
      mHistEventDensity2Ch->Fill(static_cast<Double_t>(ch_data::getCharge(chData)), static_cast<Double_t>(digit.getIntRecord().differenceInBC(mStateLastIR2Ch[ch_data::getChId(chData)])));
      ...
      if (mSetAllowedChIDs.find(static_cast<unsigned int>(ch_data::getChId(chData))) != mSetAllowedChIDs.end()) {
        ...
        if(static_cast<int>(chData.mPMNumber) == 0 && hasData[4]) mMapHistAmp1DCoincidence[0]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 1 && hasData[5]) mMapHistAmp1DCoincidence[1]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 2 && hasData[6]) mMapHistAmp1DCoincidence[2]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 3 && hasData[7]) mMapHistAmp1DCoincidence[3]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 4 && hasData[0]) mMapHistAmp1DCoincidence[4]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 5 && hasData[1]) mMapHistAmp1DCoincidence[5]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 6 && hasData[2]) mMapHistAmp1DCoincidence[6]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 7 && hasData[3]) mMapHistAmp1DCoincidence[7]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 8 && hasData[12]) mMapHistAmp1DCoincidence[8]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 9 && hasData[13]) mMapHistAmp1DCoincidence[9]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 10 && hasData[14]) mMapHistAmp1DCoincidence[10]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 11 && hasData[15]) mMapHistAmp1DCoincidence[11]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 12 && hasData[8]) mMapHistAmp1DCoincidence[12]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 13 && hasData[9]) mMapHistAmp1DCoincidence[13]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 14 && hasData[10]) mMapHistAmp1DCoincidence[14]->Fill(chData.mChargeADC);
        if(static_cast<int>(chData.mPMNumber) == 15 && hasData[11]) mMapHistAmp1DCoincidence[15]->Fill(chData.mChargeADC);
...
void DigitQcTask::endOfCycle()
{
  ...
  ILOG(Debug, Support) << "Cycle duration: NTF=" << mTfCounter << ", range = " <<(mTimeMaxNS - mTimeMinNS) / 1e6 / mTfCounter
