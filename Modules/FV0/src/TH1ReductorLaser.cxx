...
void TH1ReductorLaser::update(TObject *obj) {
  ...
  if (histo) {
    ...
    /// (checked with good_fit routine) After that the original limits of the
    /// histo must be restored
    histo->GetXaxis()->SetRangeUser(histo->GetMean() - 2. * histo->GetStdDev(),
                                    histo->GetMean());
    ...
    /// (checked with good_fit routine) After that the original limits of the
    /// histo must be restored
    histo->GetXaxis()->SetRangeUser(histo->GetMean(),
                                    histo->GetMean() + 2. * histo->GetStdDev());
