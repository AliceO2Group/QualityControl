...
bool DigitQcTask::chIsVertexEvent(const o2::fdd::ChannelData chd, bool simpleCheck)
{
  if (simpleCheck) {
    return chd.getFlag(o2::fdd::ChannelData::kIsEventInTVDC);
  } else {
...
void DigitQcTask::initialize(o2::framework::InitContext& /*ctx*/)
{
  ...
  mHistChDataBits = std::make_unique<TH2F>("ChannelDataBits", "ChannelData bits per ChannelID;Channel;Bit", sNCHANNELS_PM, 0, sNCHANNELS_PM, mMapPMbits.size(), 0, mMapPMbits.size());
  ...
  mHistTriggersSoftwareVsTCM = o2::quality_control_modules::fit::helper::registerHist<TH2F>(getObjectsManager(), PublicationPolicy::Forever, "COLZ", "TriggersSoftwareVsTCM", "Comparison of triggers from software and TCM;;Trigger name", mMapTrgBits, mapTrgValidationStatus);
...
void DigitQcTask::monitorData(o2::framework::ProcessingContext& ctx)
{
  ...
  for (auto& digit : digits) {
    ...
    Int_t pmNChanA = 0;
    Int_t pmNChanC = 0;
    Int_t pmNCoincidenceChanA = 0;
    Int_t pmNCoincidenceChanC = 0;
    Int_t pmSumTimeA = 0;
    Int_t pmSumTimeC = 0;
    Int_t pmAverTimeA = 0;
    Int_t pmAverTimeC = 0;
    ...
    mPMChargeTotalAside = static_cast<int>(mPMChargeTotalAside >> 3);
    // mPMChargeTotalCside = std::lround(static_cast<int>(mPMChargeTotalCside / 8));
    mPMChargeTotalCside = static_cast<int>(mPMChargeTotalCside >> 3);
