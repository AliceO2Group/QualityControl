  ...
  if (IsNormalizedPerDSArea == false) {
    ...
    for (auto deId : deIds) {
      ...
      for (auto dsId : dsIds) {
        ...
        double ratio = (nClusters[dsIndex] / std::abs(areas.back().first) + epsilon);
...
void svgChamber(o2::mch::contour::SVGWriter& w, int nChamber, bool bending, const TH1F* ClustersperDualSampa, o2::mch::geo::TransformationCreator transformation, bool IsNormalizedPerDSArea, bool WhiteOrGreen)
{
  ...
  double Nmax = calculateNmax(nChamber, bending, ClustersperDualSampa, IsNormalizedPerDSArea); // Maximun Ratio
