...
void* ClusterSizeReductor::getBranchAddress()
{
  return &mStats;
}

const char* ClusterSizeReductor::getBranchLeafList()
{
...
Double_t ClusterSizeReductor::getDeValue(int deid, int cathode)
{
  ...
  return mStats.deValues[cathode].values[deid];
}

void ClusterSizeReductor::update(TObject* obj)
{
  ...
  for (int xbin = 1; xbin <= h->GetXaxis()->GetNbins(); xbin++) {
    ...
    int deid = (xbin - 1) / 3;
    int cathode = (xbin - 1) % 3;

    TH1F* proj = (TH1F*)h->ProjectionY("_proj", xbin, xbin);
    auto clsize = proj->GetMean();
    mStats.deValues[cathode].values[deid] = clsize;
    delete proj;
  }
}
