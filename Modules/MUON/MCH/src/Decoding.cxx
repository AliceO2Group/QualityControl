    ...
    case headerToRead: {
      ...
      if (dsr.data == 0x1555540f00113) {
        ...
        if (gPrintLevel >= 1 || (false && ds->id == 0 && ds->header.fChipAddress == 0 && ds->header.fChannelAddress >= 30))
          fprintf(flog, "SAMPA [%2d]: Header 0x%014llx HCode %2d HPar %d PkgType %d 10BitWords %d ChipAdd %d ChAdd %2d BX %d PPar %d\n",
        ...
        if (dsg && dsg->bxc >= 0) {
          if (!BXCNT_compare(dsg->bxc, static_cast<long int>(ds->header.fBunchCrossingCounter))) {
            gNbErrors++;
            fprintf(flog, "===> ERROR SAMPA [%2d]: ChipAdd %d ChAdd %2d BX %d, expected %ld, diff %ld\n",
                    ds->id, ds->header.fChipAddress, ds->header.fChannelAddress,
...
decode_state_t Add10BitsOfData(uint64_t data, DualSampa& dsr, DualSampaGroup* /*dsg*/)
{
  ...
  switch (dsr.status) {
    case notSynchronized:
      ...
      if (gPrintLevel >= 1)
        fprintf(flog, "notSynchronized[%d]: bit=%02d  data=%013llX  %03X %03X %03X %03X %03X\n",
    ...
    case headerToRead:
      ...
      if (gPrintLevel >= 1)
        fprintf(flog, "headerToRead[%d]: bit=%02d  data=%013llX  %03X %03X %03X %03X %03X\n",
                dsr.id, dsr.bit, dsr.data,
                (int)((dsr.data >> 40) & 0x3FF),
                (int)((dsr.data >> 30) & 0x3FF),
                (int)((dsr.data >> 20) & 0x3FF),
      ...
      if (dsr.bit == 50) {
        if (dsr.data == 0x1555540f00113) {
          ...
          if (gPrintLevel >= 1)
            fprintf(flog, "SAMPA Header: HCode %2d HPar %d PkgType %d 10BitWords %d ChipAdd %d ChAdd %2d BX %d PPar %d\n",
      ...
      for (int k = 0; k < 2; k++) {
        ...
        switch (state) {
          ...
          case DECODE_STATE_HEADER_FOUND:
            ...
            if (gPrintLevel >= 1)
              fprintf(flog, "board %d %d %d -> HEADER: %05llX, %d, %d\n",
                      cru_id, link_id, i, _h,
...
void Decoder::decodeUL(uint32_t* payload_buf_32, size_t nWords, int cru_id, int dpw_id)
{
  ...
  for (size_t wi = 0; wi < nWords; wi += 1) {
    ...
    if (gPrintLevel >= 1) {
      ...
      fprintf(flog, "14 bits: %016llX\n", (value >> 50) & 0xFFF);
      fprintf(flog, "50 bits: %016llX\n", value & 0x3FFFFFFFFFFFF);
