...
///
/// \file    TrendingConfigMCH.cxx
/// \author  Nicolò Jacazio nicolo.jacazio@cern.ch
/// \author  Francesca Ercolessi francesca.ercolessi@cern.ch
/// \brief   File for the trending task configuration for the number of hits in MCH
/// \since   05/10/2021
///
...
TrendingConfigMCH::TrendingConfigMCH(std::string name, const boost::property_tree::ptree& config)
  ...
  if (const auto& customConfigs = config.get_child_optional("qc.postprocessing." + name + ".customization"); customConfigs.has_value()) {
    for (const auto& customConfig : customConfigs.value()) { // Plot configuration
      ...
      if (const auto& customNames = customConfig.second.get_child_optional("name"); customNames.has_value()) {
        if (customConfig.second.get<std::string>("name") == "NamePrefix") {
          mConfigTrendingFECHistRatio.namePrefix = customConfig.second.get<std::string>("value");
        } else if (customConfig.second.get<std::string>("name") == "TitlePrefix") {
          mConfigTrendingFECHistRatio.titlePrefix = customConfig.second.get<std::string>("value");
        }
      }
    }
  }
