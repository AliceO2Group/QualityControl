...
bool MapFEC::readPadMapping(uint32_t de, std::string bMapfile, std::string nbMapfile, bool newMapping)
{
  ...
  while (!filebend.eof()) {
    filebend >> address >> padx >> pady >> x >> y >> dsid >> manuch;
  ...
  while (!filenbend.eof()) {
    filenbend >> address >> padx >> pady >> x >> y >> dsid >> manuch;
...
bool MapFEC::readPadMapping2(uint32_t de, bool newMapping)
{
  ...
  for (int pad = 0; pad < MCH_PAD_ADDR_MAX; pad++) {
    ...
    float padX = segment.padPositionX(pad);
    float padY = segment.padPositionY(pad);
    float padSizeX = segment.padSizeX(pad);
    float padSizeY = segment.padSizeY(pad);
    ...
    mPadMap[de][address].fDE = de;
    mPadMap[de][address].fDsID = dsid;
    mPadMap[de][address].fAddress = pad;
    //mPadMap[de][address].fPadx = padx;
    //mPadMap[de][address].fPady = pady;
    mPadMap[de][address].fX = padX;
    mPadMap[de][address].fY = padY;
    mPadMap[de][address].fSizeX = padSizeX;
    mPadMap[de][address].fSizeY = padSizeY;
    mPadMap[de][address].fCathode = segment.isBendingPad(pad) ? 0 : 1;
    mPadMap[de][address].fBad = 0;
  }
  return true;
}
...
{
  std::map<int, std::unique_ptr<o2::mch::mapping::Segmentation>>::iterator si = segmentations.find(de);
  o2::mch::mapping::Segmentation* segment = nullptr;
  if (si == segmentations.end()) {
    segment = new o2::mch::mapping::Segmentation(de);
    std::unique_ptr<o2::mch::mapping::Segmentation> sp(segment);
    segmentations.insert(make_pair(de, std::move(sp)));
  } else {
    segment = si->second.get();
  }
  if (!segment)
    return false;
