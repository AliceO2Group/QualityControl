...
/// \file   MUON/MID/GBTRawDataChecker.h
...
class GBTRawDataChecker
{
 public:
  ...
  struct GBT {
    std::vector<o2::mid::ROBoard> regs{}; /// Regional boards
    std::vector<o2::mid::ROBoard> locs{}; /// Local boards
 ...
 private:
  ...
  bool checkEvent(bool isTriggered, const std::vector<o2::mid::ROBoard>& regs, const std::vector<o2::mid::ROBoard>& locs);
  bool checkEvents(bool isTriggered);
  bool checkConsistency(const o2::mid::ROBoard& board);
  bool checkConsistency(const std::vector<o2::mid::ROBoard>& boards);
  bool checkMasks(const std::vector<o2::mid::ROBoard>& locs);
  bool checkLocalBoardSize(const o2::mid::ROBoard& board);
  bool checkLocalBoardSize(const std::vector<o2::mid::ROBoard>& boards);
  bool checkRegLocConsistency(const std::vector<o2::mid::ROBoard>& regs, const std::vector<o2::mid::ROBoard>& locs);
  uint8_t getElinkId(const o2::mid::ROBoard& board) const;
  unsigned int getLastCompleteTrigEvent();
  bool isCompleteSelfTrigEvent(const o2::InteractionRecord& ir) const;
  std::string printBoards(const std::vector<o2::mid::ROBoard>& boards) const;
  ...
  uint32_t mSyncTrigger{ o2::mid::raw::sORB };    /// Trigger for synchronization
