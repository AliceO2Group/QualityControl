...
/// \file   MUON/MID/GBTRawDataChecker.cxx
...
void GBTRawDataChecker::init(uint16_t feeId, uint8_t mask)
{
  ...
  bool GBTRawDataChecker::checkLocalBoardSize(const o2::mid::ROBoard& board)
{
  ...
  bool GBTRawDataChecker::checkLocalBoardSize(const std::vector<o2::mid::ROBoard>& boards)
{
  ...
  bool GBTRawDataChecker::checkConsistency(const o2::mid::ROBoard& board)
{
  /// Checks that the event information is consistent

  bool isSoxOrReset = board.triggerWord & (o2::mid::raw::sSOX | o2::mid::raw::sEOX | o2::mid::raw::sRESET);
  bool isCalib = o2::mid::raw::isCalibration(board.triggerWord);
  bool isPhys = board.triggerWord & o2::mid::raw::sPHY;
  ...
  bool GBTRawDataChecker::checkConsistency(const std::vector<o2::mid::ROBoard>& boards)
{
  /// Checks that the event information is consistent
  for (auto& board : boards) {
  ...
  bool GBTRawDataChecker::checkMasks(const std::vector<o2::mid::ROBoard>& locs)
{
  /// Checks the masks
  for (auto loc : locs) {
    // The board patterns coincide with the masks ("overwritten" mode)
    if (loc.statusWord & o2::mid::raw::sOVERWRITTEN) {
  ...
  bool GBTRawDataChecker::checkRegLocConsistency(const std::vector<o2::mid::ROBoard>& regs, const std::vector<o2::mid::ROBoard>& locs)
{
  /// Checks consistency between local and regional info
  uint8_t regFired{0};
  for (auto& reg : regs) {
    uint8_t ireg = o2::mid::raw::getLocId(reg.boardId) % 2;
    // auto busyItem = mBusyFlagSelfTrig.find(8 + ireg); // unused variable
  ...
  if (problems) {
    ...
    uint8_t busy{ 0 };
    for (uint8_t iboard = 0; iboard < o2::mid::crateparams::sNELinksPerGBT; ++iboard) {
      auto busyItem = mBusyFlagSelfTrig.find(iboard);
      if (busyItem != mBusyFlagSelfTrig.end() && busyItem->second) {
        busy |= (iboard < o2::mid::crateparams::sMaxNBoardsInLink) ? (1 << iboard) : (0xF << (4 * (iboard % 2)));
      }
    }
  ...
  std::string GBTRawDataChecker::printBoards(const std::vector<o2::mid::ROBoard>& boards) const
{
  /// Prints the boards
  std::stringstream ss;
  for (auto& board : boards) {
  ...
  bool GBTRawDataChecker::checkEvent(bool isTriggered, const std::vector<o2::mid::ROBoard>& regs, const std::vector<o2::mid::ROBoard>& locs)
{
  ...
  uint8_t GBTRawDataChecker::getElinkId(const o2::mid::ROBoard& board) const
{
  /// Returns the e-link ID
  if (o2::mid::raw::isLoc(board.statusWord)) {
...
void GBTRawDataChecker::clearChecked(bool isTriggered, bool clearTrigEvents)
{
  ...
  for (auto& lastIdxItem : lastIndexes) {
    size_t firstIdx = lastIdxItem.second + 1;
...
bool GBTRawDataChecker::checkEvents(bool isTriggered)
{
  ...
  for (auto& evtIdxItem : orderedIndexes) {
    ...
    for (auto& evtPair : evtIdxItem.second) {
      ...
      // uint8_t triggerId = boardInfo.board.triggerWord; // unused variable
      auto elinkId = getElinkId(boardInfo.board);

      bool isBusy = ((boardInfo.board.statusWord & o2::mid::raw::sREJECTING) != 0);
      busyRaised |= isBusy;
      ...
      if (o2::mid::raw::isLoc(boardInfo.board.statusWord)) {
  ...
  bool GBTRawDataChecker::process(gsl::span<const o2::mid::ROBoard> localBoards, gsl::span<const o2::mid::ROFRecord> rofRecords, gsl::span<const o2::mid::ROFRecord> pageRecords)
{
  /// Checks the raw data
  mDebugMsg.clear();
  ...
  for (auto rofIt = rofRecords.begin(); rofIt != rofRecords.end(); ++rofIt) {
    ...
    for (auto locIt = localBoards.begin() + rofIt->firstEntry; locIt != localBoards.begin() + rofIt->firstEntry + rofIt->nEntries; ++locIt) {
      ...
      if (locIt->statusWord & o2::mid::raw::sREJECTING) {
        auto& selfVec = mBoardsSelfTrig[id];
        auto ir = rofIt->interactionRecord;
        if (id >= o2::mid::crateparams::sMaxNBoardsInLink) {
      ...
      // Compute the masks
      if (locIt->triggerWord & o2::mid::raw::sSOX) {
        if (o2::mid::raw::isLoc(locIt->statusWord)) {
