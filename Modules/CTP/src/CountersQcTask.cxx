...
void CTPCountersTask::monitorData(o2::framework::ProcessingContext& ctx)
{
  ...
  if (!pp) {LOG(info) << "no payload pointer ";} 
  ...
  for(int i=1; i < tokens.size(); i++) {
     ...
     for(int j=0;j<sCheck.length();j++) { if (!isdigit(sCheck[j])) notNumber = true; } 
  ...
  if (firstCycle) {
    ...
    for (int i=0; i<48; i++) mPreviousTrgInput.push_back(trgInput[i]);
    mTime.push_back(0.);
    mInputRate.push_back(0.);
    for (int i=0; i<48; i++) mTimes[i].push_back(0.);
    for (int i=0; i<48; i++) mInputRates[i].push_back(0.);
  } else {
