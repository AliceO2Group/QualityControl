...
RawTask::~RawTask()
{
  ...
  for (auto& [trg, histos] : mRawAmplitudeEMCAL) {
    for (auto h : histos) {
  ...
  for (auto& [trg, histos] : mRawAmplMaxEMCAL) {
    for (auto h : histos) {
      delete h;
    }
  }

  for (auto& [trg, histos] : mRawAmplMinEMCAL) {
    for (auto h : histos) {
      delete h;
    }
  }
  for (auto& [trg, histos] : mRMSperSM) {
    for (auto h : histos) {
      delete h;
    }
  }

  for (auto& [trg, histos] : mMEANperSM) {
    for (auto h : histos) {
      delete h;
    }
  }

  for (auto& [trg, histos] : mMAXperSM) {
    for (auto h : histos) {
      delete h;
    }
  }
  for (auto& [trg, histos] : mMINperSM) {
    for (auto h : histos) {
      delete h;
    }
  }
