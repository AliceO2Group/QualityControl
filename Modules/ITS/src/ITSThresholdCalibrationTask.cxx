...
void ITSThresholdCalibrationTask::monitorData(o2::framework::ProcessingContext& ctx)
{
  ...
  if (scanType == 'V') iScan = 0;
    else if (scanType == 'I') iScan = 1;
         else if (scanType == 'T') iScan = 2;
  ...
  for (auto StaveStr: splitRes){
     ...
     if (result.status==1) SuccessStatus[result.Layer]++;
...
Int_t ITSThresholdCalibrationTask:: getBarrel(Int_t iLayer){
     ...
     Int_t iBarrel;
     if (iLayer<3) iBarrel=0;
     else if (iLayer<5) iBarrel=1;
          else iBarrel=2;
