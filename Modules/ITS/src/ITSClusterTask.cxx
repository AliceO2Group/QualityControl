...
#pragma omp parallel for schedule(dynamic)
  ...
  for (Int_t iLayer = 0; iLayer < NLayer; iLayer++) {
      ...
      if (!mEnableLayers[iLayer]) continue;
      ...
      memset(mClusterOccupancyOBmonitor, 0, sizeof(mClusterOccupancyOBmonitor));
       ...
       for (Int_t iLayer = 0; iLayer < NLayer; iLayer++) {
          ...
          for (Int_t iStave = 0; iStave < mNStaves[iLayer]; iStave++){
             ...
             if (iLayer < 3) for (Int_t iChip = 0; iChip < mNChipsPerHic[iLayer]; iChip++) hClusterSizeIBmonitor[iLayer][iStave][iChip]->Reset();
            
             else for (Int_t iHic = 0; iHic < mNHicPerStave[iLayer]; iHic++) hClusterSizeOBmonitor[iLayer][iStave][iHic]->Reset();
...
*/
      ...
      hOccupancyOB[iLayer]->SetTitle(Form("ClusterOccupancyInLastNROFS %d", iLayer));
      ...
      formatAxes(hAverageClusterOBmonitor[iLayer], "Chip Number", "Stave Number", 1, 1.10);
