  ...
  : PostProcessingConfig(name, config)
{
  for (const auto& plotConfig : config.get_child("qc.postprocessing." + name + ".plots")) {
    if (const auto& sourceNames = plotConfig.second.get_child_optional("names"); sourceNames.has_value()) {
      ...
      //for (const auto& sourceName : sourceNames.value() && const auto& sourceVarexp : sourceVarexps.value() && const auto& sourceTitle : sourceTitles.value()) {
      while (itname != sourceNames.value().end() || itexp != sourceVarexps.value().end() || ittitle != sourceTitles.value().end()) {
        plots.push_back({ itname->second.data(),
                          ittitle->second.data(),
                          itexp->second.data(),
  ...
  for (const auto& dataSourceConfig : config.get_child("qc.postprocessing." + name + ".dataSources")) {
    if (const auto& sourceNames = dataSourceConfig.second.get_child_optional("names"); sourceNames.has_value()) {
      ...
      const auto& sourceReductors = dataSourceConfig.second.get_child_optional("reductorNames"); //take all types
      const auto& sourceModules = dataSourceConfig.second.get_child_optional("moduleNames");     //take all paths
      ptree::const_iterator itname = sourceNames.value().begin();
      ...
      while (itname != sourceNames.value().end() || ittype != sourceTypes.value().end() || itpath != sourcePaths.value().end() ||
             itred != sourceReductors.value().end() || itmod != sourceModules.value().end()) {
        dataSources.push_back({ ittype->second.data(),
                                itpath->second.data(),
                                itname->second.data(),
                                itred->second.data(),
                                itmod->second.data() });
